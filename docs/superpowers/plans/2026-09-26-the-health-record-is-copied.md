...
A final review, after the store and the reader were built, changed these rules too:

22. **Permissions asked for before use:** `HealthConnectReader.dayTotals` checks granted permissions once per call and puts an ungranted metric straight into `TotalsResult.failed`, with no call and no log, so a stored figure for it stands; `session(...)` skips the distance and active-calories aggregate the same way when that permission is not granted, checked once per batch of records translated rather than once per session.
23. **The problem log moved off the caller's dispatcher:** `HealthRecordSync.note` is now a suspend function that wraps the write in `Dispatchers.IO`, because `copyNow` runs on whatever dispatcher its caller is on (Main, in the app), and the log is a synchronous file write.
24. **Rule 5, amended: `SecurityException` is transient too.** Health Connect throws it for a read made while the app is in the background, not only for a genuinely refused permission; treating it as final would end a catch-up over nothing, and the history limit still shows up as empty weeks regardless. `HealthRecordSyncTest`'s history-limit test now refuses with `IllegalArgumentException`, and a new test covers `SecurityException` staying transient on an old slice.
25. **Wording — kinds not allowed:** phrased as "Reading X is not allowed — tap Connect to allow it/them," so one wording covers one name or several without asking each `HealthKind` whether its display name reads as singular or plural. The three-or-fewer threshold is `HealthRecordWording.MOST_NAMED`, a named choice.
26. **Wording — the status date:** `HealthRecordWording.status` takes a `zone: ZoneId = ZoneId.systemDefault()` parameter and now names the year ("from 6 August 2025") when the earliest date's year is not the year "now" (in that zone) falls in; tests pass a fixed UTC zone.
27. **Settings' status rule extracted and tested directly:** `HealthRecordState.from(days, earliest, syncRows, granted)` in `HealthPorts.kt` is the pure function behind what `RoomHealthRecordStatus` shows; `RoomHealthRecordStatus` now only reads the rows and calls it. Its KDoc's quoted line was also corrected to what Settings actually shows.
28. **Export orders a night's stages:** the `@Relation` fetch behind `SleepDao.allNights()` makes no promise about a night's stages' order, so `BackupRepository.export` sorts them itself, by start time and then by row id, before writing.
29. **Restore racing a running copy:** accepted as rare and self-healing. `BackupRepository.restore` clears the copying bookmarks inside its transaction; a copy that is mid-flight at that same moment could still save a bookmark back afterwards, which the next expired token or window re-read heals.
30. **`WINDOW_DAYS`'s comment corrected:** it is a choice matching the documented 30-day life of a Health Connect changes token, not — as it previously said — a measured read quota.
