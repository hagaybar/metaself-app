 ...
 * and [inMillilitres] only choose the wording: an amount of millilitres is said in ml (D56).
 */
@Composable
fun AmountTooMuch(tooMuch: Boolean, most: Double, countedAs: CountedAs, inMillilitres: Boolean = false) {
    if (!tooMuch) return
    val words = BelievableAmount.words(most)
    Text(
        text = when {
            inMillilitres -> stringResource(R.string.amount_at_most_ml, words)
