...
/*
 * Changing how much of something was eaten, on an item already on the record — what *Correct this
 * item* does when the amount is typed over (D44).
 *
 * Two slices are one slice twice, and half a risotto is half of everything in it. It is deliberately
 * linear, and therefore handles a bigger bowl and not a richer one.
 *
 * Scaling is from the item it is given. The screen keeps the item as it was logged and always
 * scales from that, so going back arrives exactly where it started rather than at the compounded
 * rounding of two multiplications.
 */
